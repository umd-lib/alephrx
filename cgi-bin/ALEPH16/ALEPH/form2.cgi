...
while (@row = $sth->fetchrow_array) {
	      ...
	      print "<TD BGCOLOR=\"#FFFF99\"><FONT SIZE=-1>&nbsp;$twoweeksago</TD>\n";
	      print "<TD BGCOLOR=\"#F0F8FF\"><FONT SIZE=-1>&nbsp;$maxstamp</TD>\n";
	      print "<TD BGCOLOR=\"#FFFF99\"><FONT SIZE=-1>&nbsp;$reportdate</TD>\n";
	      print "<TD BGCOLOR=\"#F0F8FF\" ALIGN=\"CENTER\"><FONT SIZE=-1></TD></TR>\n";
...
if ($URGENT) {
    $filter = "and report.status = 'Assigned'";
}
...
sub match {
    ...
    $statement = "select report.summary, people.phone, people.name, reply.text, reply.name from report, people, reply WHERE people.id = report.id and report.id = reply.parent_id and report.summary = '$summary' and  people.phone = '$phone' and people.name = '$name' and reply.text = '$mresponse' and reply.name = '$rname'";
...
## sets the background color of the summary cell based on the status of the report
##

sub cell_background  {

    if ($row[4] eq "user input needed" and $maxstamp gt $reportdate) {
	$cellbk = "#FF00FF" }

    elsif ($row[4] eq "pending" and $replystamp lt $oneweekago ) {
	$cellbk = "#FF00FF" }   

    elsif ($row[4] eq "user_input needed" and $reportdate + 90 lt $now ) {
	$cellbk = "#FF00FF" }

    elsif ($row[4] eq "pending" and $maxstamp gt $twoweeksago) {
	$cellbk = "#FF00FF" }


    elsif ($row[4] eq "new" and $reportdate < $twoweeksago ) {
	$cellbk = "#FF00FF" }
...
$dbh = DBI->connect("DBI:mysql:$database:$db_server", $user, $password);
$statement_8 =   "select to_days(reply.date), itd, to_days(NOW()), to_days(DATE_SUB(NOW(),INTERVAL 14 DAY)), to_days(DATE_SUB(NOW(), INTERVAL 7 DAY)), to_days(DATE_SUB(NOW(), INTERVAL 1 DAY)), to_days(report.date) from reply,report where reply.parent_id = '$row_id' and report.id = reply.parent_id";
